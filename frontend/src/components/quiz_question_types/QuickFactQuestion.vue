...
const generateOptions = () => {
  ...
  for (const card of props.allCards) {
    if (card.id !== props.card.id && card.quick_facts) {
      // Filter out facts that exist on the current card
      const uniqueFacts = card.quick_facts.filter(f => !props.card.quick_facts?.includes(f))
      wrongFacts.push(...uniqueFacts)
    }
  }
...
const checkAnswer = (option: string) => {
  ...
  const isCorrect = props.card.quick_facts?.includes(option)
  if (isCorrect) {
    emit('correct')
  } else {
...
<template>
  <div class="question-content">
    <div class="debug-header">
      <span class="question-type-badge">Quick Fact Question</span>
    ...
    <div class="question-prompt">
      <div class="question-type">
        ...
        <h3>Which fact belongs to this card?</h3>
